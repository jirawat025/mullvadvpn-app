...
pub fn init_logger(
    ...
    let stdout_formatter = tracing_subscriber::fmt::layer().with_ansi(true);
    ...
    // TODO: FIX
    if output_timestamp {
        let file_formatter = tracing_subscriber::fmt::layer().with_ansi(false);
        ...
        let file_formatter = file_formatter.with_writer(non_blocking_file_appender);
        reg.with(
